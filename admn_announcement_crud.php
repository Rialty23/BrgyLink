...
<div class="p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <button onclick="openAddModal()"
            class="bg-info text-white px-5 py-2 rounded-lg shadow">
            + Add Announcement
        </button>
    </div>
    ...
    <div class="bg-white shadow-lg rounded-xl overflow-x-auto">
        <table class="w-full text-sm">
            <thead class="bg-gray-100 text-gray-700">
                <tr>
                    <th class="px-4 py-3 text-left">Set Date</th>
            ...
            <?php if (is_array($view) && count($view) > 0): ?>
                <?php foreach ($view as $row): ?>
                    <tr class="border-t hover:bg-gray-50">
                        <td class="px-4 py-3 font-medium text-gray-800"><?= htmlspecialchars($row['set_date']) ?></td>
...
<div id="addModal" class="fixed inset-0 bg-black/50 hidden flex items-center justify-center z-50">
    <div class="bg-white rounded-xl p-6 w-full max-w-xl">
        ...
        <form method="post" enctype="multipart/form-data" class="space-y-4">
            ...
            <input type="date" name="set_date" required class="w-full border p-3 rounded" placeholder="Set Date">
            ...
            <div class="flex justify-end gap-2">
                <button type="button" onclick="closeAddModal()" class="bg-red-400 text-white px-4 py-2 rounded">Cancel</button>
...
<div id="editModal" class="fixed inset-0 bg-black/50 hidden flex items-center justify-center z-50">
    <div class="bg-white rounded-xl p-6 w-full max-w-xl">
        ...
        <form method="post" enctype="multipart/form-data" class="space-y-4">
            ...
            <label>Set Date:</label>
            <input type="date" name="edit_set_date" id="edit_set_date" required class="w-full border p-3 rounded" placeholder="Set Date">
            ...
            <div class="flex justify-end gap-2">
                <button type="button" onclick="closeEditModal()" class="bg-red-400 text-white px-4 py-2 rounded">Cancel</button>
...
function openEditModal(id, title, event, photo, set_date){
    document.getElementById('edit_id').value = id;
    document.getElementById('edit_title').value = title;
    document.getElementById('edit_event').value = event;
    document.getElementById('edit_set_date').value = set_date;
