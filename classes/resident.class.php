...
");
            ...
            $checkStmt->execute([$fname, $lname, $bdate]);
        ...
        ");
            ...
            INSERT INTO tbl_resident (
                ...
                voter, pwd, indigent, single_parent, pregnancy, malnourished, four_ps,
                    senior_citizen, out_of_school_youth, lgbtq, valid1, valid2, qr_code, is_walk_in, status2
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ");
...
");
    ...
    ";
            ...
            ------------------------------ */
                ...
                $message = $response['success']
                    ...
                    : "Request approved, but email failed: {$response['error']}";



                echo "
            <script>
                alert('Resident added successfully!\\nControl No: $control_no  $message');
                window.open('receipt.php?id=$lastId', '_blank');
    ...
    public function view_resident()
    {
        ...
        if (isset($_GET['deleted'])) {
            ...
            $stmt = $connection->prepare(
             ...
             WHERE status2 = 'DELETED' OR request_status='rejected'
             ORDER BY id_resident DESC"
            );
        } else {
    ...
    public function update_resident()
    {
        if (isset($_POST['update_resident'])) {
            ...
            $stmt = $connection->prepare("UPDATE `tbl_resident` SET 
                    ...
                    `lgbtq`=?, `senior_citizen`=?, `addedby`=?, `request_status`=?, `is_walk_in`=?, `status2`=?
                    {$password_update}
                    WHERE `id_resident` = ?");
    ...
    public function approve_request()
    {
        if (isset($_POST['approve_request'])) {
            ...
            if ($resident && !empty($resident['email'])) {
                ...
                $message = "Request approved, but resident email not found.";
            }
    ...
    public function reject_request()
    {
        if (isset($_POST['reject_request'])) {
            ...
            if ($reject_reason === '') {
                echo "<script type='text/javascript'>alert('Please provide a rejection reason.');</script>";
                header("Refresh:0");
                return;
            }

            $reject_reason = substr($reject_reason, 0, 128);

            // Open the database connection
            $connection = $this->openConn();

            // Prepare and execute the SQL query
            $stmt = $connection->prepare("UPDATE tbl_resident SET request_status = 'rejected', rejected_reason = ? WHERE id_resident = ?");
            ...
            if ($resident && !empty($resident['email'])) {
                ...
                $message = $response['success']
                    ...
                    : "Request rejected, but email failed: {$response['error']}";
            } else {
                $message = "Request rejected, but resident email not found.";
            }
    ...
    {
        $connection = $this->openConn();
        $stmt = $connection->prepare("SELECT * FROM tbl_resident WHERE request_status='pending' AND status2!='DELETED'  order by id_resident DESC ");
        $stmt->execute();
        $result = $stmt->fetchAll(PDO::FETCH_ASSOC);
        return $result;
    }
