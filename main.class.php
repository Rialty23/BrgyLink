...
require __DIR__ . '/../email/PHPMailer/PHPMailer.php';
require __DIR__ . '/../email/PHPMailer/SMTP.php';
require __DIR__ . '/../email/PHPMailer/Exception.php';

class BMISClass
{

    private function resolveStoragePath($path)
    {
        if (empty($path)) {
            return '';
        }
        ...
        return __DIR__ . '/../' . ltrim(str_replace('\\', '/', $path), '/');
    }

    private function getReceiptTitleByControlNo($controlNo)
    {
        if (strpos($controlNo, 'TRBSPERMIT-') === 0) {
            return 'Business Permit Receipt';
        }
        if (strpos($controlNo, 'TRBRGYCLEARANCE-') === 0) {
            return 'Barangay Clearance Receipt';
        }
        if (strpos($controlNo, 'TRINDIGENCY-') === 0) {
            return 'Indigency Receipt';
        }
        if (strpos($controlNo, 'TRRESIDENCY-') === 0) {
            return 'Residency Receipt';
        }
        if (strpos($controlNo, 'TRBLOTTER-') === 0) {
            return 'Blotter Receipt';
        }

        return 'Barangay Receipt';
    }

    private function saveReceiptPng($title, $controlNo, $fullName, $qrPath)
    {
        ...
        $header = imagecolorallocate($img, 7, 50, 95);
        $text = imagecolorallocate($img, 40, 40, 40);
        $muted = imagecolorallocate($img, 90, 90, 90);

        imagefilledrectangle($img, 0, 0, $width, $height, $white);
        imagefilledrectangle($img, 0, 0, $width, 130, $header);
        imagestring($img, 5, 30, 55, $title, $white);

        imagestring($img, 5, 40, 180, 'Transaction No: ' . $controlNo, $text);
        imagestring($img, 5, 40, 235, 'Full Name: ' . strtoupper($fullName), $text);
        imagestring($img, 5, 40, 290, 'Date Issued: ' . date('F d, Y'), $text);

        imageline($img, 40, 340, $width - 40, 340, $muted);

        $resolvedQrPath = $this->resolveStoragePath($qrPath);
        if (!empty($resolvedQrPath) && file_exists($resolvedQrPath)) {
            $qrImg = @imagecreatefrompng($resolvedQrPath);
            if ($qrImg) {
                imagecopyresampled($img, $qrImg, 325, 390, 0, 0, 250, 250, imagesx($qrImg), imagesy($qrImg));
                imagedestroy($qrImg);
            }
        }

        imagestring($img, 4, 180, 710, 'This document serves as an official proof of barangay registration.', $muted);
        imagestring($img, 5, 340, 780, 'Barangay Office', $header);

        $receiptFile = $receiptDir . $controlNo . '.png';
        imagepng($img, $receiptFile);
        imagedestroy($img);
    }

    private function saveReceiptPngByControlNo($controlNo, $fullName, $qrPath)
    {
        $title = $this->getReceiptTitleByControlNo($controlNo);
        $this->saveReceiptPng($title, $controlNo, $fullName, $qrPath);
    }
    ...
    public function sendEmail($data)
    {
        ...
        try {
            ...
            // Sender & recipient
            $mail->setFrom('penta2914@gmail.com', 'BARANGAYLINK Automated Email');
            $mail->addAddress($data['email']);

            // Content
            $mail->isHTML(true);
            $mail->Subject = $data['subject'];
            $mail->Body    = "<h3>{$data['title']}</h3><p>{$data['message']}</p>";
    ...
    public function create_announcement()
    {
        if (isset($_POST['create_announce'])) {
            ...
            INSERT INTO tbl_announcement (id_announcement, title, event, start_date, addedby, photo, set_date)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
            $stmt->execute([$id_announcement, $title, $event, $start_date, $addedby, $photo_path, $set_date]);
        ...
        if (isset($_POST['update_announce'])) {
            ...
            if (!empty($_FILES['photo']['name'])) {
                ...
                $stmt = $connection->prepare("UPDATE tbl_announcement 
                                          SET title = ?, event = ?, start_date = ?, addedby = ?, photo = ?, set_date = ?
                                          WHERE id_announcement = ?");
                $stmt->execute([$title, $event, $start_date, $addedby, $photo_path, $set_date, $id_announcement]);
            } else {
                // No new photo uploaded, keep old
                $stmt = $connection->prepare("UPDATE tbl_announcement 
                                          SET title = ?, event = ?, start_date = ?, addedby = ?, set_date = ?
                                          WHERE id_announcement = ?");
                $stmt->execute([$title, $event, $start_date, $addedby, $set_date, $id_announcement]);
            }
    ...
    public function create_certofres()
    {
        ...
        if (isset($_POST['create_certofres'])) {
            ...
            $attachment = implode(',', $uploadedFiles);
   ...
   <script>
    ...
    //     return $view;
    // }
    public function view_certofres($archived = false)
    {
        $connection = $this->openConn();
  ...
  SELECT control_no
   ...
   <script>
    ...
    public function view_certofindigency($archived = false)
    {
        $connection = $this->openConn();
   ...
   <script>
    ...
    public function view_clearance($archived = false)
    {
        $connection = $this->openConn();
    ...
    public function create_bspermit()
    {
        ...
        ");
            ...
            $qr_data = "Transaction No: " . $control_no . "\n"
                     . "Full Name: " . strtoupper($lname) . ", " . strtoupper($fname) . " " . strtoupper($mi) . "\n"
                     . "Business Name: " . $bsname . "\n"
                     . "Business Industry: " . $bsindustry . "\n"
                     . "Address: " . strtoupper($houseno) . " " . strtoupper($street) . ", " . strtoupper($brgy) . ", " . strtoupper($municipal) . "\n"
                     . "Contact: " . $contact . "\n"
                     . "Date Issued: " . date('F d, Y');
            QRcode::png($qr_data, $qrFile, QR_ECLEVEL_M, 4);
            ...
            INSERT INTO tbl_bspermit (
                ...
                bsname, houseno, street, brgy, municipal, bsindustry, aoe, qr_code, control_no, contact, email, bcode
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        ");
    ...
    public function view_bspermit($archived = false)
    {
        $connection = $this->openConn();
    ...
    public function update_bspermit()
    {
        if (isset($_POST['update_bspermit'])) {
            ...
            bsindustry = ?, aoe = ?, contact = ?, email = ?, bcode = ? WHERE id_bspermit = ?");
            $stmt->execute([$id_bspermit, $lname, $fname, $mi,  $bsname, $houseno,  $street, $brgy, $municipal, $bsindustry, $aoe, $contact, $email, $bcode]);
    ...
    public function create_blotter()
    {
        ...
        if (isset($_POST['create_blotter'])) {
            ...
            // -----------------------------
            // Generate control number from tbl_blotter
            // -----------------------------
            $stmtCN = $connection->prepare("
            SELECT control_no 
            FROM tbl_blotter 
        ...
        ");
            ...
            $qr_data = "Transaction No: " . $control_no . "\n"
                ...
                . "Respondent's Details:\n"
                        . "Full Name: " . strtoupper($rlname) . ", " . strtoupper($rfname) . " " . strtoupper($rmi) . "\n"
                        . "Address: " . $raddress . "\n"
                        . "Contact: " . $rcontact . "\n"
            ...
            `houseno`, `street`,`brgy`, `municipal`, `contact`, `narrative`, `rlname`, `rfname`, `rmi`, `rage`, `rcontact`, `raddress`, `type`, `date`, `time`, `location`, `witness`, `control_no`, `qr_code`)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

            $stmt->execute([$id_blotter, $id_resident, $lname, $fname, $mi, $houseno,  $street, $brgy, $municipal, $contact, $narrative, $rlname, $rfname, $rmi, $rage, $rcontact, $raddress, $type, $date, $time, $location, $witness, $control_no, $qrFile]);
    ...
    public function create_blotter_walkin()
    {
        if (isset($_POST['create_blotter_walkin'])) {
            ...
            $narrative       = trim(htmlspecialchars($_POST['narrative']));
                        ...
                        /* Respondent details */
            $rlname = trim(htmlspecialchars($_POST['rlname'] ?? ''));
            $rfname = trim(htmlspecialchars($_POST['rfname'] ?? ''));
            $rmi = trim(htmlspecialchars($_POST['rmi'] ?? ''));

            $rage = trim(htmlspecialchars($_POST['rage'] ?? ''));
            $rcontact = trim(htmlspecialchars($_POST['rcontact'] ?? ''));
        ...
        ");
                ...
                (`case_no`, `lname`, `fname`, `mi`, `houseno`, `street`, `brgy`,
                 `municipal`, `blot_photo`, `contact`, `narrative`, `rlname`, `rfname`, `rmi`, `rage`, `rcontact`, `raddress`, `type`, `date`, `time`, `location`, `witness`, `timeapplied`,
                 `control_no`, `qr_code`)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
    ...
    public function view_blotter($archived = false)
    {
        $connection = $this->openConn();
